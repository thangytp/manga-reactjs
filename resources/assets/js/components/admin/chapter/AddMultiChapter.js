...
export default class AddMultiChapter extends Component {
	constructor(props){
		super(props);
	...
	componentDidMount(){
		document.title = 'Add Multiple Chapter';
	...
	handleChangeContentURL(e){
		...
		fetch(pageToVisit)
	  	...
	  	.then(function(data) {
		    ...
		    if(source === 'mangak'){
			    let linkHtml = $('.chapter-list').html(); 
			    let links = $(linkHtml).find('div span').html();
			    console.log(links);
	...
	render(){
		...
		return (
				<div>
					<Breadcrumb activePage="Add multiple chapter" />
					<div className="card">
